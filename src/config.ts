...
export function readConfig(env: Record<string, string | undefined> = process.env) {
  ...
  const requireOrigin = env.REQUIRE_ORIGIN ?? "off";
  if (requireOrigin !== "on" && requireOrigin !== "off") throw new Error("REQUIRE_ORIGIN must be on or off");
  ...
  const config = {
    ...
    entryDecider, control: control === "on", corsOrigin, requireOrigin: requireOrigin === "on", maxSseClients: num("MAX_SSE_CLIENTS", 2000, 1, 20000), ledger, persistAllEvents: ledgerEvents === "all", dataDir: env.DATA_DIR ?? `data/${dataMode === "demo" ? "demo" : "paper"}`,
